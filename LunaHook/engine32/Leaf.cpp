...
static void SpecialHookLeaf(hook_stack* stack,  HookParam *, uintptr_t *data, uintptr_t *split, size_t*len)
{
...
namespace{
bool hook2(hook_stack*s,void* data1, size_t* len,uintptr_t*role)
  {   
    strReplace(save,"\\k","");
...
bool InsertLeafHook()
{
  ...
  hp1.address=addr2+7;
  hp1.hook_before=hook2;
  hp1.hook_after=hook2a;
  hp1.type=EMBED_ABLE|EMBED_DYNA_SJIS;
...
namespace{
  bool kizuato(){
    ...
    hp.text_fun =[](hook_stack* stack,  HookParam *, uintptr_t *data1, uintptr_t *split, uintptr_t *len1){
      *len1=0;
      ...
      last[ret]=current;
      auto data=new char[current.size()+1];strcpy(data,current.c_str());
      size_t _l=current.size();size_t*len=&_l;
      StringReplacer((char*)data,len,"\\k\\n",2,"\n",1);
      StringReplacer((char*)data,len,"\\n",2,"",0);
      StringReplacer((char*)data,len,"\\k",2,"",0);
      StringReplacer((char*)data,len,"\\s",2,"",0);
      StringFilterBetween((char*)data,len,"|",1,">",1);
      StringReplacer((char*)data,len,"<F60",4,"",0); 
      StringReplacer((char*)data,len,"<R",2,"",0); 
      StringReplacer((char*)data,len,">",1,"",0); 
      *len1=_l;*data1=(DWORD)data;
      // current=std::string((char*)data,*len);
      // current=std::regex_replace(current,std::regex("<F60(.*?)>"),"$1");
      // current=std::regex_replace(current,std::regex("<R(.*?)|(.*?)>"),"$1");
      //strcpy((char*)data,current.c_str());*len=current.size();
