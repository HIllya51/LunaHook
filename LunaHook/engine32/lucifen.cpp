...
namespace{
 ...
 bool hookBefore_navel(hook_stack*s,void* data, size_t* len,uintptr_t*role)
  {  
 ...
 void hookafter_navel(hook_stack*s,void* data, size_t len)
  {  
      ...
      std::string  newData = std::string((char*)data,len); 
...
//   unsigned int v53; // [esp+1Ch] [ebp+4h]
    ...
    hp.hook_before=hookBefore_navel;
...
namespace Private {
  ...
  bool dispatchScenarioText(char *text, ULONG split,hook_stack*s,void* data, size_t* len1,uintptr_t*role)
  {
    // text[0] could be \0
...
bool dispatchNameTextafter(char *text, ULONG split,hook_stack*s,void* data, uintptr_t  len1 )
  ...
  void dispatchScenarioTextafter(char *text, ULONG split,hook_stack*s,void* data, uintptr_t len1 )
  { 
    auto scenarioEndAddress = (LPSTR *)(text + 0x1000);
    auto scenarioEnd = *scenarioEndAddress;
    if (!Engine::isAddressReadable(scenarioEnd))
  ...
  //  bool b1=  dispatchNameText(self + nameOffset_, retaddr,s,data,len1,role);
    bool b2=dispatchScenarioText(self + scenarioOffset_, retaddr,s,data,len1,role);
    return  b2;
  }
    void hookafter(hook_stack*s,void* data, uintptr_t len1)
  {
    auto self = (LPSTR)s->ecx;
    ULONG retaddr = s->stack[0];
   //  dispatchNameTextafter(self + nameOffset_, retaddr,s,data,len1);
     dispatchScenarioTextafter(self + scenarioOffset_, retaddr,s,data,len1); 
  }
} // namespace Private
  ...
  size_t  countZero(const char *s, size_t limit=1500)
{
  ...
  void hookafter(hook_stack*s,void* data, size_t len1){
    ...
    enum { role = Engine::OtherRole };std::string oldData = text ;
