		...
		for (int i = 0; i < depth; ++i)
		{
			...
			if (IthGetMemoryRange((void*)addr, nullptr, nullptr))
			{
				if (strlen((char*)addr) > 9)
				{
					HookParam hp;
					hp.type = DIRECT_READ;
					if (funcAddr == funcW) hp.type |= CODEC_UTF16;
					hp.address = addr;
...
namespace Private {
  ...
  public:
     ...
     */
    void dispatchText(LPSTR text, bool paddingSpace,void* data, size_t* len,uintptr_t*role)
    {
    ...
    void dispatchTextList2(LPSTR text, bool paddingSpace,std::string newData1){
      ...
      if(save.size()!=newdata.size())return ;
      int i=0;
      for (auto p = text; p; p = nextText(p)) { 
        std::string oldData = p;
        auto newData=newdata[i];i++;
        if (newData != oldData) {
    ...
    void dispatchTextList(LPSTR text, bool paddingSpace,void* data, size_t* len,uintptr_t*role)
    {
    ...
    *role=argaddr;
    ...
    if (Engine::isAddressWritable(textAddress)) {
      ...
      if (arg->isTextList(text)) {
        if (backtrackText_) // old shinario games have re-translate problems
          return true;
        arg->dispatchTextList(text,backtrackText_,data,len,role);
      } else
        arg->dispatchText(text, backtrackText_,data,len,role);
      return true;
      if (backtrackText_ && Engine::isAddressWritable(charAddress)) {
        if (textAddress - text == 2) { // for wide character
          if ((signed char)textAddress[-2] < 0) {
